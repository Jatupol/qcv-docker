// client/src/components/report/OverallOQAReportBase.tsx
// ===== BASE COMPONENT FOR OVERALL OQA DPPM REPORTS =====
...
import { type DefectLegendItem, type OQADPPMOverallRecord } from '../../types/report';
// ============ TYPE DEFINITIONS ============

export interface OQADPPMOverallData extends OQADPPMOverallRecord {
  isHighlighted?: string;
}

export interface DataSourceFunctions {
  fetchChartData: (params: any) => Promise<any>;
  fetchDefectData: (params: any) => Promise<any>;
  fetchFiscalYears: () => Promise<any>;
  fetchWorkWeeks: (year: string) => Promise<any>;
}

export interface OverallOQAReportConfig {
  // Report metadata
  title: string;
  ...
  hideUI?: {
    ...
    header?: boolean;
    fiscalYearCard?: boolean;
    larCard?: boolean;
    totalNGCard?: boolean;
  };
}

export interface OverallOQAReportBaseProps {
  config: OverallOQAReportConfig;
}

// ============ MAIN COMPONENT ============

const OverallOQAReportBase: React.FC<OverallOQAReportBaseProps> = ({ config }) => {
  const [loading, setLoading] = useState(false);
  const [oqaData, setOqaData] = useState<OQADPPMOverallData[]>([]);
  ...
  // Filter states - initialize with defaults if provided
  const [fiscalYear, setFiscalYear] = useState<string>(config.defaultDateRange?.yearFrom || '');
  const [ww, setWw] = useState<string>(config.defaultDateRange?.wwFrom || '');
    ...
    const fetchWorkWeeks = async () => {
      if (fiscalYear) {
        ...
        if (wwResult.success && wwResult.data) {
          ...
          // Validate current ww exists in list, fallback to latest available
          const currentWwNum = ww ? parseInt(ww, 10).toString() : '';
          const matchingWw = wwResult.data.find((item: string) => parseInt(item, 10).toString() === currentWwNum);
          if (matchingWw) {
            setWw(matchingWw);
          } else {
            // Current WW not in list - use latest available
            const sortedWeeks = [...wwResult.data].sort((a: string, b: string) => parseInt(b, 10) - parseInt(a, 10));
            if (sortedWeeks.length > 0) {
              console.log(`🔄 Setting ww to latest available: "${sortedWeeks[0]}"`);
              setWw(sortedWeeks[0]);
            } else {
              setWw('');
            }
          ...
          console.warn('⚠️ Failed to fetch work weeks:', wwResult.message);
        }
      } else {
  ...
  const fetchReportData = async () => {
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        for (const record of result.data) {
          if (!monthMap.has(record.yearmonth)) {
            monthMap.set(record.yearmonth, {
              ...
              dppmTarget: record.dppmTarget || 150,
        ...
        console.log('📊 Defect API Response:', defectResult);
  ...
  const renderFilters = () => {
    ...
    const handleExportPDF = async () => {
      ...
      try {
        ...
        const filename = `Overall_OQA_DPPM_Report_${dateStr}.pdf`;
        pdf.save(filename);
  ...
  const renderSummaryCards = () => {
    ...
    const currentLAR = currentData ? currentData.lar : 0;

    // Calculate total NG from raw defect data
    ...
    const matchingDefects = rawDefectData.filter(defectRecord => {
      ...
      return monthWW === paddedWW;
    });

    const totalNG = matchingDefects.reduce((sum, defectRecord) => {
      return sum + parseInt(defectRecord.ng_qty || 0);
    }, 0);

    const fyww = fiscalYear && ww ? `${fiscalYear}/WW${ww.padStart(2, '0')}` : ww || 'N/A';

    // Filter visible cards based on hideUI config
    const visibleCards = [];

    if (!config.hideUI?.fiscalYearCard) {
      visibleCards.push(
        <div key="fyww" className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 hover:shadow-xl">
    ...
    if (!config.hideUI?.larCard) {
      visibleCards.push(
        <div key="lar" className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 hover:shadow-xl">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm font-semibold text-purple-100 uppercase tracking-wide">LAR %</p>
              <p className="text-3xl font-bold text-white mt-2 drop-shadow-md">{currentLAR != null ? currentLAR.toFixed(2) : '0.00'}%</p>
            </div>
    ...
    if (!config.hideUI?.totalNGCard) {
      visibleCards.push(
        <div key="ng" className="bg-gradient-to-br from-red-500 to-orange-500 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 hover:shadow-xl">
          <div className="flex items-center justify-between">
            <div className="flex-1">
    ...
    // Only render grid if there are visible cards
    if (visibleCards.length === 0) {
      return null;
    }

    return (
      <div className={`grid grid-cols-1 md:grid-cols-${visibleCards.length} gap-4`}>
        {visibleCards}
      </div>
    );
  };
  ...
  const renderDataTable = () => {
    const metrics = [
      { key: 'dppm', label: 'DPPM', format: (val: number) => val != null ? Math.round(val).toString() : '0' },
      { key: 'dppmTarget', label: 'DPPM Target (150)', format: (val: number) => val != null ? Math.round(val).toString() : '150' },
      { key: 'lar', label: '%LAR', format: (val: number) => val != null ? `${val.toFixed(2)}%` : '0.00%' },
    ];
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody>
              ...
              {dynamicDefectLegend.map((defectType, defectIndex) => {
                ...
                const isEvenRow = (defectIndex + metrics.length) % 2 === 0;
                return (
                  <tr key={defectType.name} className="border-b border-gray-200 hover:bg-slate-100 transition-colors">
                    <td
                      className="px-4 py-2.5 text-sm font-semibold text-white border-r border-gray-300 text-left"
                    ...
                    {oqaData.map((item, index) => {
                      ...
                      return (
                        <td
                          key={`${item.yearmonth}-${index}`}
                          className={`px-3 py-2.5 text-center text-sm font-medium text-slate-700 border-r border-gray-300 ${
  ...
  console.log('🎨 Rendering OverallOQAReportBase - loading:', loading, 'oqaData.length:', oqaData.length);
...
export default OverallOQAReportBase;
