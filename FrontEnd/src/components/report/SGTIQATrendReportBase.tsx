...
export interface DataSourceFunctions {
  ...
  fetchModels: (params?: { year?: string; ww?: string; onlyWeekOfProduct?: boolean }) => Promise<APIResponse<string[]>>;
...
// ============ CUSTOM X-AXIS TICK ============
// Splits "WW45 May '26" into two lines; keeps short labels (e.g. "Dec '25") on one line.
const CustomXAxisTick = ({ x, y, payload }: any) => {
  const value: string = payload?.value ?? '';
  const parts = value.split(' ');
  const isWeekly = parts.length >= 3 && parts[0].startsWith('WW');
  const line1 = isWeekly ? parts[0] : value;
  const line2 = isWeekly ? parts.slice(1).join(' ') : null;
...
const SGTIQATrendReportBase: React.FC<SGTIQATrendReportBaseProps> = ({ config }) => {
  ...
  const [onlyWeekOfProduct, setOnlyWeekOfProduct] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const refreshModels = async () => {
      if (fiscalYear) {
        const params: { year?: string; ww?: string; onlyWeekOfProduct?: boolean } = { year: fiscalYear };
        if (ww) params.ww = ww;
        if (onlyWeekOfProduct) params.onlyWeekOfProduct = true;
    ...
    refreshModels();
  }, [fiscalYear, ww, onlyWeekOfProduct]);
  ...
  // Auto-fetch when any filter changes (non-sharedFilters mode)
  useEffect(() => {
    if (config.sharedFilters) return;
    if (!initialLoadDone) return;
    const showDateFilters = config.showDateFilters !== false;
    const dateRequired = config.dateRequired !== false;
    const showModelFilter = config.showModelFilter !== false;
    const modelRequired = config.modelRequired || false;
    if (showDateFilters && dateRequired && (!fiscalYear || !ww)) return;
    if (showModelFilter && modelRequired && selectedModels.length === 0) return;
    fetchSGTData();
  }, [fiscalYear, ww, selectedModels, selectedProductType, onlyWeekOfProduct]);
  ...
  const fetchSGTData = async (directFY?: string, directWW?: string, directModels?: string[]) => {
    try {
      ...
      if (onlyWeekOfProduct) queryParams.onlyWeekOfProduct = true;
  ...
  const getOptimizedHtml2CanvasOptions = (element: HTMLElement, identifier?: string, highResolution: boolean = false, isTable: boolean = false, isChart: boolean = false) => ({
    scale: highResolution ? (isChart ? 2.0 : 1.5) : 0.2,
    logging: false,
    useCORS: true,
    allowTaint: true,
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Create PDF with landscape orientation to fit wide charts and tables
      ...
      // Add filter info matching what is shown on screen
      let filterText = '';
      pdf.setFontSize(8);
      if (fiscalYear && ww) {
        filterText += `Period: FY${fiscalYear} WW${ww}`;
      }
      if (selectedProductType) {
        filterText += ` | Product Type: ${selectedProductType}`;
      }
      if (onlyWeekOfProduct) { 
         filterText += ' | Filter: Only week of product';
      }
      ...
      if (selectedModels.length > 0) {
        const modelsText = `Models: ${selectedModels.join(', ')}`;
        const splitModels = pdf.splitTextToSize(modelsText, pageWidth - 2 * margin);
        pdf.text(splitModels, margin, yPosition);
        yPosition += 6 * splitModels.length;
      }
      ...
      console.log('⏳ Waiting for all sections to render...');
      await new Promise(resolve => setTimeout(resolve, 300));

      const maxContentHeight = pageHeight - margin - footerHeight;

      // Build ordered list of all sections matching the screen order:
      // 1. Overall  2. Per product type  3. Per individual model
      const sections: Array<{ chartSelector: string; tableSelector: string; label: string }> = [];

      sections.push({
        chartSelector: '[data-chart="overall"]',
        tableSelector: '[data-table="Overall"]',
        label: 'Overall'
      });

      const seenProductTypes = new Set<string>();
      sgtDataByModel.forEach(md => {
        const pt = md.data[0]?.product_type;
        if (pt && pt.trim() !== '' && !seenProductTypes.has(pt)) {
          seenProductTypes.add(pt);
          sections.push({
            chartSelector: `[data-chart="producttype-${pt}"]`,
            tableSelector: `[data-table="producttype-${pt}"]`,
            label: `Product Type: ${pt}`
          });
        }
      });

      sgtDataByModel.forEach(md => {
        if (md.model && md.model.trim() !== '') {
          sections.push({
            chartSelector: `[data-chart="${md.model}"]`,
            tableSelector: `[data-table="${md.model}"]`,
            label: `Model: ${md.model}`
          });
        }
      });

      const totalSections = sections.length;
      const progressPerSection = 70 / totalSections;

      for (let i = 0; i < sections.length; i++) {
        const section = sections[i];
        const currentProgress = 20 + (i * progressPerSection);
        setExportProgress(Math.round(currentProgress));

        const chartElement = contentRef.current?.querySelector(section.chartSelector) as HTMLElement;
        const tableElement = contentRef.current?.querySelector(section.tableSelector) as HTMLElement;

        if (chartElement && tableElement) {
          console.log(`📦 Exporting section ${i + 1}/${totalSections}: ${section.label}`);

          const [chartCanvas, tableCanvas] = await Promise.all([
            html2canvas(chartElement, getOptimizedHtml2CanvasOptions(chartElement, section.chartSelector, true, false, true) as any),
            html2canvas(tableElement, getOptimizedHtml2CanvasOptions(tableElement, section.tableSelector, true, true, false) as any)
          ]);

          setExportProgress(Math.round(currentProgress + (progressPerSection * 0.6)));
          ...
          setExportProgress(Math.round(currentProgress + progressPerSection));
        }

        // Page break between sections (except after the last one)
        if (i < sections.length - 1 && yPosition > margin) {
          pageNumber++;
          pdf.addPage('l');
          yPosition = margin;
        }
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 px-4 py-3 mb-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* Only Week of Product */}
          <div className="flex items-center gap-1.5">
            <input
              id="onlyWeekOfProduct"
              type="checkbox"
              checked={onlyWeekOfProduct}
              onChange={(e) => setOnlyWeekOfProduct(e.target.checked)}
              className="w-3.5 h-3.5 accent-purple-600 cursor-pointer"
            />
            <label htmlFor="onlyWeekOfProduct" className="text-xs font-semibold text-gray-600 cursor-pointer select-none whitespace-nowrap">
              Only week of product
            </label>
          </div>
          <div className="w-px h-6 bg-gray-300" />
  ...
  if (loading) {
    return (
      <div className={config.hideUI?.breadcrumb && config.hideUI?.filters ? "p-6 flex items-center justify-center" : "min-h-screen bg-gray-50 px-3 py-4 flex items-center justify-center"}>
  ...
  if (!loading && sgtDataByModel.length === 0) {
    return (
      <div className={config.hideUI?.breadcrumb && config.hideUI?.filters ? "p-6" : "min-h-screen bg-gray-50 px-3 py-4"}>
  ...
  return (
    <div className={config.hideUI?.breadcrumb && config.hideUI?.filters ? "" : "min-h-screen bg-gray-50 px-3 py-4"}>
      <div className="w-full">
        {/* Breadcrumb */}
