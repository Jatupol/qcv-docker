...
/**
 * Check if resend email button should be shown for a defect record.
 *
 * Email sending conditions (mirrors backend logic):
 * OQA  — Abnormal defect ≥1 | Normal repeat problem & area ≥3
 * SIV  — Abnormal defect ≥1 | Normal repeat problem & area ≥3 | Normal defect (round ≥ 2 only)
 * OBA  — Normal defect | Abnormal defect ≥1 | Normal repeat problem & area ≥3
 */
const shouldShowResendEmailButton = (defect: { inspection_no: string; defect_type?: string; round?: number }): boolean => {
  const { inspection_no, defect_type, round } = defect;
  if (!inspection_no || !defect_type) return false;

  const inspectionPrefix = inspection_no.substring(0, 3).toUpperCase();

  const oqaDefectTypes = ['Abnormal defect ≥1', 'Normal repeat problem & area ≥3'];
  const obaDefectTypes = ['Normal defect', 'Abnormal defect ≥1', 'Normal repeat problem & area ≥3'];

  if (inspectionPrefix === 'OQA') {
    return oqaDefectTypes.includes(defect_type);
  } else if (inspectionPrefix === 'SIV') {
    if (oqaDefectTypes.includes(defect_type)) return true;
    if (defect_type === 'Normal defect' && (round ?? 0) >= 2) return true;
    return false;
      ...
      {(showDelete || showResendEmail) && defect.id && (
        <td className="border border-red-300 px-2 py-2 text-center">
          <div className="flex items-center justify-center gap-1">
            {showResendEmail && shouldShowResendEmailButton(defect) && (
              <button
