...
const DefectsPage: React.FC = () => {
  ...
  const customFormFields = (selectedEntity: Defect | null) => {
    ...
    return (
      ...
      <div className="md:col-span-1">
        <label htmlFor="report_order" className="block text-sm font-medium text-gray-700 mb-1">
          Report Order
        </label>
        <input
          type="number"
          id="report_order"
          name="report_order"
          defaultValue={selectedEntity?.report_order ?? ''}
          min={1}
          max={9999}
        ...
        />
        <p className="mt-1 text-xs text-gray-500">Order sequence for displaying defects in reports (lower = first)</p>
      </div>
      <div className="md:col-span-1">
  ...
  const extractFormData = (formData: FormData) => {
    const reportOrderRaw = formData.get('report_order') as string;
    const reportOrder = reportOrderRaw ? parseInt(reportOrderRaw, 10) : undefined;
    return {
      report_order: reportOrder && !isNaN(reportOrder) ? reportOrder : undefined,
