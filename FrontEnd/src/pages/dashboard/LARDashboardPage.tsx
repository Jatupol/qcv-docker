...
const LARDashboardPage: React.FC = () => {
  ...
  const calculateSummary = (data: LARDashboardRecord[]) => {
    ...
    const aboveTarget = latestData.filter(d => d.status === 'Achieve').length;
  ...
  const getTableData = () => {
    ...
    const months = Array.from(new Set(larData.map(d => d.month_year)))
      .sort()
      .map(m => ({
        key: m,
    ...
    const getRowKey = (d: LARDashboardRecord) =>
      `${d.model}|${d.lar_achieve_threshold ?? ''}|${d.lar_accept_min_threshold ?? ''}|${d.lar_accept_max_threshold ?? ''}|${d.lar_abnormal_threshold ?? ''}`;

    // Filter by latest month or show all
    const filteredData = latestMonthOnly
      ? larData.filter(d => d.month_year === months[months.length - 1]?.key)
      : larData;
    ...
    const models = uniqueKeys.map(key => {
      ...
      // Create months object with LAR values and status
  ...
  const getLatestMonthLabel = (): string => {
    ...
    const latestMonth = larData.map(d => d.month_year).sort().pop()!;
