// client/src/pages/dashboard/DPRMDashboardPage.tsx
// ===== DPPM DASHBOARD WITH FILTERS =====
...
interface DPPMDashboardRecord {
  period_key: string;
  dimension_value: string;
  dppm_achieve_threshold?: number | null;
  dppm_accept_min_threshold?: number | null;
  dppm_accept_max_threshold?: number | null;
  dppm_abnormal_threshold?: number | null;
  dppm: number;
  status?: string | null;
}
...
const DPPMDashboardPage: React.FC = () => {
  ...
  // Data states
  const [dppmData, setDppmData] = useState<DPPMDashboardRecord[]>([]);
  ...
  const fetchDPPMData = async (overrideDates?: { dateFrom?: string; dateTo?: string }) => {
    setLoading(true);
    setError(null);
    ...
    try {
      const result = await reportService.getDPPMDashboard({
        dateFrom: from,
        dateTo: to,
      ...
      if (result.success && result.data) {
        // Ensure DPPM values are numbers
        const normalizedData = result.data.map(item => ({
          ...item,
          dppm: typeof item.dppm === 'string' ? parseFloat(item.dppm) : item.dppm,
          dppm_achieve_threshold: item.dppm_achieve_threshold ?
            (typeof item.dppm_achieve_threshold === 'string' ? parseFloat(item.dppm_achieve_threshold) : item.dppm_achieve_threshold) :
            undefined,
          dppm_accept_min_threshold: item.dppm_accept_min_threshold ?
            (typeof item.dppm_accept_min_threshold === 'string' ? parseFloat(item.dppm_accept_min_threshold) : item.dppm_accept_min_threshold) :
            undefined,
          dppm_accept_max_threshold: item.dppm_accept_max_threshold ?
            (typeof item.dppm_accept_max_threshold === 'string' ? parseFloat(item.dppm_accept_max_threshold) : item.dppm_accept_max_threshold) :
            undefined,
          dppm_abnormal_threshold: item.dppm_abnormal_threshold ?
            (typeof item.dppm_abnormal_threshold === 'string' ? parseFloat(item.dppm_abnormal_threshold) : item.dppm_abnormal_threshold) :
            undefined
        }));
        setDppmData(normalizedData);
        calculateSummary(normalizedData);
      } else {
        setError(result.message || 'Failed to fetch DPPM data');
      }
    } catch (err) {
  ...
  const calculateSummary = (data: DPPMDashboardRecord[]) => {
    if (data.length === 0) {
      setSummaryData({ totalModels: 0, modelsAchieve: 0, modelsAccept: 0, modelsAbnormal: 0, averageDPPM: 0 });
      return;
    }
    ...
    const achieve = groupBy === 'model' ? latestData.filter(d => d.status === 'Achieve').length : 0;
    ...
    const overallAvg = latestData.reduce((sum, d) => sum + d.dppm, 0) / latestData.length;
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      ...
      setDateFrom(from);
      setDateTo(to);
      fetchDPPMData({ dateFrom: from, dateTo: to });
    }
  ...
  const formatDPPM = (value: number | string) => {
    const numValue = typeof value === 'string' ? parseFloat(value) : value;
    return formatNumber(numValue.toString(),0);
  };
  ...
  const getTableData = () => {
    if (dppmData.length === 0) return { models: [], months: [] };

    const months = Array.from(new Set(dppmData.map(d => d.period_key)))
      .sort()
      .map(m => ({ key: m, label: formatPeriodKey(m, timePeriod) }));

    const getRowKey = (d: DPPMDashboardRecord) =>
      groupBy === 'model'
        ? `${d.dimension_value}|${d.dppm_achieve_threshold ?? ''}|${d.dppm_accept_min_threshold ?? ''}|${d.dppm_accept_max_threshold ?? ''}|${d.dppm_abnormal_threshold ?? ''}`
        : d.dimension_value;

    const filteredData = latestMonthOnly
      ? dppmData.filter(d => d.period_key === months[months.length - 1]?.key)
      : dppmData;
    const uniqueKeys = Array.from(new Set(filteredData.map(d => getRowKey(d)))).sort();

    const models = uniqueKeys.map(key => {
      const rowData = dppmData.filter(d => getRowKey(d) === key);
      const first = rowData[0];
      const thresholds = {
        achieve: first.dppm_achieve_threshold,
        acceptMin: first.dppm_accept_min_threshold,
        acceptMax: first.dppm_accept_max_threshold,
        abnormal: first.dppm_abnormal_threshold
      };

      const monthsData: { [key: string]: { value: number; status?: string | null } | null } = {};
      months.forEach(month => {
        const record = rowData.find(d => d.period_key === month.key);
        monthsData[month.key] = record ? { value: record.dppm, status: record.status } : null;
      });
  ...
  const getLatestPeriodLabel = (): string => {
    if (dppmData.length === 0) return '';
    const latest = dppmData.map(d => d.period_key).sort().pop()!;
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-8">
      ...
      <Card variant="elevated" className="relative overflow-hidden">
        ...
        <div className="flex items-center">
          ...
          <div>
            <p className="text-2xl font-bold text-gray-900">{Math.round(summaryData.averageDPPM)}</p>
                ...
                <tr key={item.key} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  {tableData.months.map((month) => {
                    ...
                    return (
                      ...
                      >
                        {cell !== null ? formatDPPM(cell.value) : ''}
                      </td>
                    );
                  })}
                </tr>
