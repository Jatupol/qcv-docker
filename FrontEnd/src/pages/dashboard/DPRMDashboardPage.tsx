...
const DPPMDashboardPage: React.FC = () => {
  ...
  const getTableData = () => {
    ...
    const months = Array.from(new Set(dppmData.map(d => d.month_year)))
      .sort()
      .map(m => ({
        key: m,
    ...
    const getRowKey = (d: DPPMDashboardRecord) =>
      `${d.model}|${d.dppm_achieve_threshold ?? ''}|${d.dppm_accept_min_threshold ?? ''}|${d.dppm_accept_max_threshold ?? ''}|${d.dppm_abnormal_threshold ?? ''}`;

    // Filter by latest month or show all
    const filteredData = latestMonthOnly
      ? dppmData.filter(d => d.month_year === months[months.length - 1]?.key)
      : dppmData;
    ...
    const models = uniqueKeys.map(key => {
      ...
      // Create months object with DPPM values and status
  ...
  const getLatestMonthLabel = (): string => {
    ...
    const latestMonth = dppmData.map(d => d.month_year).sort().pop()!;
