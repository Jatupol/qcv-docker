...
interface UnderkillDashboardRecord {
  ...
  underkill_achieve_threshold?: number | null;
  underkill_accept_min_threshold?: number | null;
  underkill_accept_max_threshold?: number | null;
  underkill_abnormal_threshold?: number | null;
  underkill: number;
  status?: string | null;
}
...
const UnderKillDashboardPage: React.FC = () => {
  ...
  const fetchUnderkillData = async (overrideDates?: { dateFrom?: string; dateTo?: string }) => {
    setLoading(true);
    setError(null);
  ...
  const calculateSummary = (data: UnderkillDashboardRecord[]) => {
    if (data.length === 0) {
      setSummaryData({ totalModels: 0, modelsAchieve: 0, modelsAccept: 0, modelsAbnormal: 0, averageUnderkill: 0 });
      return;
    }
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      ...
      setDateFrom(from);
      setDateTo(to);
      fetchUnderkillData({ dateFrom: from, dateTo: to });
    }
  ...
  const getTableData = () => {
    ...
    const months = Array.from(new Set(underkillData.map(d => d.period_key)))
      .sort()
      .map(m => ({ key: m, label: formatPeriodKey(m, timePeriod) }));

    const getRowKey = (d: UnderkillDashboardRecord) =>
      groupBy === 'model'
        ? `${d.dimension_value}|${d.underkill_achieve_threshold ?? ''}|${d.underkill_accept_min_threshold ?? ''}|${d.underkill_accept_max_threshold ?? ''}|${d.underkill_abnormal_threshold ?? ''}`
        : d.dimension_value;

    const filteredData = latestMonthOnly
      ? underkillData.filter(d => d.period_key === months[months.length - 1]?.key)
      : underkillData;
  ...
  const getLatestPeriodLabel = (): string => {
    ...
    const latest = underkillData.map(d => d.period_key).sort().pop()!;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Button
                variant="ghost"
                disabled
                className="flex items-center opacity-40 cursor-not-allowed"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
