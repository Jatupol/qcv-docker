...
const SOQMDailyPage: React.FC = () => {
  ...
  const [selectedPartsites, setSelectedPartsites] = useState<string[]>([]);

  // Pagination state (to compute Seq# correctly across pages and sorts)
  ...
  const partsiteOptions = useMemo(() => {
    const sites = [...new Set(data.map(r => r.partsite).filter((s): s is string => !!s))].sort();
    return sites.map(s => ({ value: s, label: s }));
  }, [data]);

  const filteredData = useMemo(() => {
    if (selectedPartsites.length === 0) return data;
    return data.filter(r => r.partsite && selectedPartsites.includes(r.partsite));
  }, [data, selectedPartsites]);

  // ============ SUMMARY CARDS (react to Production Site filter) ============

  const summary = useMemo(() => {
    if (filteredData.length === 0) return null;

    const totalNG = filteredData.reduce((sum, r) => sum + Number(r.total_ng), 0);
    const uniqueWeeks = new Set(filteredData.map(r => r.ww));
    const uniqueModels = new Set(filteredData.map(r => r.model));
    const uniqueLots = new Set(filteredData.map(r => r.lotno));

    // Top Lot NG
    const lotNGMap: Record<string, number> = {};
    filteredData.forEach(r => { lotNGMap[r.lotno] = (lotNGMap[r.lotno] || 0) + Number(r.total_ng); });
    ...
    filteredData.forEach(r => { modelNGMap[r.model] = (modelNGMap[r.model] || 0) + Number(r.total_ng); });
  ...
  const columns: ColumnsType<SOQMDailyRecord> = [
    {
      ...
      width: 65,
      align: 'center',
      render: (_val: unknown, _record: SOQMDailyRecord, index: number) =>
    ...
    },
    {
      ...
      filters: [...new Set(filteredData.map((r) => r.ww))].sort((a, b) => a - b).map((ww) => ({ text: String(ww), value: ww })),
    ...
    },
    {
      ...
      width: 130,
      align: 'center',
      sorter: (a, b) => (a.partsite || '').localeCompare(b.partsite || ''),
    },
    {
      title: 'Model',
      dataIndex: 'model',
      ...
      filters: [...new Set(filteredData.map((r) => r.model))].sort().map((m) => ({ text: m, value: m })),
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div className="w-52">
                <div className="flex items-center justify-between mb-0.5">
                  <label className="text-xs font-semibold text-purple-600">Model</label>
                  {selectedModels.length > 0 && (
                    <button
                      onClick={() => setSelectedModels([])}
                      className="text-xs text-purple-400 hover:text-purple-700 leading-none"
                    >
                      Clear
                    </button>
                  )}
              ...
              {/* Production Site */}
              <div className="w-44">
                <label className="block text-xs font-semibold text-teal-600 mb-0.5">Production Site</label>
                <MultiSelect
                  options={partsiteOptions}
                  selected={selectedPartsites}
                  onChange={(v) => { setSelectedPartsites(v); setCurrentPage(1); }}
                  placeholder="All Sites"
                  disabled={partsiteOptions.length === 0}
                  color="teal"
                />
              </div>
              ...
              <h3 className="text-base font-semibold text-gray-900">
                ...
                {data.length > 0 && (
                  <span className="ml-2 text-sm font-normal text-gray-500">
                    ({filteredData.length}{filteredData.length !== data.length ? ` / ${data.length}` : ''} records)
                  </span>
                )}
              </h3>
            </div>
