...
const OverviewOQACustomerPage: React.FC = () => {
  ...
  const handleNgQtyClick = useCallback(async (partnoCustomer: string, defectGroup: string) => {
    if (!selectedFY || !selectedWW) return;
    setLotModalTitle(`${partnoCustomer} / ${defectGroup}`);
    setLotModalVisible(true);
    setLotModalLoading(true);
    try {
      const result = await getOverviewOQALotDetail(selectedFY, selectedWW, partnoCustomer, defectGroup);
      if (result.success && result.data) {
  ...
  const pivotedData = useMemo((): PivotedRow[] => {
    // Group rawData by partno_customer
    ...
    rawData.forEach(row => {
      if (!productMap.has(row.partno_customer)) {
        productMap.set(row.partno_customer, {
          firstRow: row,
          defects: new Map(),
        });
      }
      const entry = productMap.get(row.partno_customer)!;
      if (row.defect_group && row.defect_group !== 'No Defect') {
    ...
    // Sort products by partno_customer
  ...
  const grandTotals = useMemo(() => {
    ...
    rawData.forEach(row => {
      if (!productMap.has(row.partno_customer)) {
        productMap.set(row.partno_customer, row);
      }
    });
    ...
    const fixedLeft: ColumnsType<PivotedRow> = [
      ...
      },
      {
        ...
        onCell: (record) => {
          ...
          const fail = record.total_lot_fail;
          const ng = record.total_ng;
          const highlight = (fail !== 0 && fail / 2 > ng) || (fail === 0 && ng > 0);
      ...
      render: (_: unknown, record: PivotedRow) => {
        ...
        if (record.rowType === 'dppm') {
          ...
          if (ngVal > 0) {
            return (
              <a
                onClick={() => handleNgQtyClick(record.partno_customer, dg)}
  ...
  const fixedColCount = 8; // 8 product cols
            ...
            return (
              <Table.Summary fixed>
                ...
                {(() => {
                  ...
                  const highlightFailNg = (fail !== 0 && fail > ng) || (fail === 0 && ng > 0);
                  ...
                  return (
                    <Table.Summary.Row>
                      ...
                      <Table.Summary.Cell index={3} align="right">
                        {cell(highlightFailNg ? '#FFFF00' : '#fff',
                          <span style={{ color: highlightFailNg ? '#FF0000' : undefined, fontWeight: 600 }}>
            ...
            columns={[
              ...
              },
              {
                ...
                render: (val: string, record: any) => (
                  <a
                    href={`/inspection/oqa?inspection_no=${encodeURIComponent(record.inspection_no)}&lotno=${encodeURIComponent(record.lotno)}`}
                    target="_blank"
