...
const InspectionStationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Live search for Round
    if (filters.round && filters.round.trim() !== '') {
      filtered = filtered.filter(record =>
        (record.samplingRound?.toString() || '').includes(filters.round!.trim())
      );
    }
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Round */}
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-gray-600">Round:</span>
              <input
                type="text"
                className={`px-2 py-1 text-sm border rounded focus:outline-none focus:ring-1 w-16 ${
                  (filters.round || '').trim()
                    ? 'border-blue-400 bg-blue-50 text-blue-900 font-medium'
                    : 'border-gray-300 bg-white'
                } ${theme.focusRing}`}
            ...
            {/* Clear Button */}
            {(filters.judgment !== 'all' || filters.model !== 'All' || filters.partsite !== 'All' || filters.lotno.trim() || filters.fvilineno.trim() || (filters.round || '').trim()) && (
              <>
                <div className="w-px h-6 bg-gray-300" />
                <button
                  onClick={() => {
