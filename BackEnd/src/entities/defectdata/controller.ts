...
export class DefectDataController extends GenericSpecialController<DefectData> implements ISpecialController {
   ...
   * 3. SIV inspections: Send email if defect_type is 'Abnormal defect ≥1' or 'Normal repeat problem & area ≥3'
   *    Also: SIV + 'Normal defect' triggers email ONLY when round >= 2 (round check done in sendDefectEmail)
   *
  ...
  private shouldSendDefectEmail(inspection_no: string, defect_type?: string): boolean {
    ...
    const sivDefectTypes = [
      'Abnormal defect ≥1',
      'Normal repeat problem & area ≥3',
      'Normal defect'  // Round >= 2 required; final check done in sendDefectEmail
    ];
    ...
    if (inspectionPrefix === 'OQA') {
      ...
      return sivDefectTypes.includes(defect_type);
  ...
  private async sendDefectEmail(defectId: number): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      // SIV + Normal defect: only send when round >= 2
      const inspectionPrefixForRound = inspectionNo.substring(0, 3).toUpperCase();
      if (inspectionPrefixForRound === 'SIV' && defectType === 'Normal defect') {
        const roundNum = parseInt(round) || 0;
        if (roundNum < 2) {
          console.log(`⏭️ Skipping SIV Normal defect email: round=${round} < 2 (defect ID: ${defectId})`);
          return { success: false, message: `Round ${round} does not meet condition (≥2) for SIV Normal defect` };
        }
      }
