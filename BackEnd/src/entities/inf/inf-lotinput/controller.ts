...
export class InfLotInputController {
  ...
  getMissingItems = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      console.log('🔍 GET /api/inf-lotinput/missing-items');
      const result = await this.service.getMissingItems();
      res.status(200).json(result);
    } catch (error) {
