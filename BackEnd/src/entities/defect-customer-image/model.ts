// server/src/entities/defect-customer-image/model-drizzle.ts
/* Defect Customer Image Entity Model - Drizzle ORM Implementation
 * SERIAL Primary Key Pattern with bytea data
 *
 * This is the Drizzle ORM version of the DefectImageModel (customer).
 ...
 * - Enabled via DRIZZLE_DEFECT_CUSTOMER_IMAGE=true in .env
 */
...
import {
  defectCustomerImage,
  DefectCustomerImage as DrizzleDefectCustomerImage,
  NewDefectCustomerImage
} from '../../db/schema';
import {
  DefectImage,
 ...
 */
function mapDrizzleToEntity(row: DrizzleDefectCustomerImage): DefectImage {
  return {
    id: row.id,
...
// ==================== DEFECT CUSTOMER IMAGE MODEL CLASS (DRIZZLE) ====================

/**
 * Defect Customer Image Entity Model - Drizzle ORM Version
 *
 * Data access layer for defect customer image management using Drizzle ORM.
 * Provides image storage operations with type-safe queries.
 */
export class DefectCustomerImageModel {
  ...
  /**
   * Create a new defect customer image
   */
  async create(data: CreateDefectImageRequest): Promise<DefectImage> {
    const insertData: NewDefectCustomerImage = {
  ...
  /**
   * Bulk create defect customer images
   */
  ...
  async getById(id: number): Promise<DefectImage | null> {
    const result = await this.db
      .select()
      .from(defectCustomerImage)
      .where(eq(defectCustomerImage.id, id))
      .limit(1);
  ...
  async getAll(): Promise<DefectImage[]> {
    const result = await this.db
      .select()
      .from(defectCustomerImage)
      .orderBy(desc(defectCustomerImage.id));
  ...
  async getByDefectId(defectId: number): Promise<DefectImage[]> {
    const result = await this.db
      .select()
      .from(defectCustomerImage)
      .where(eq(defectCustomerImage.defectId, defectId))
      .orderBy(asc(defectCustomerImage.id));
...
/**
 * Factory function to create a Drizzle defect customer image model instance
 */
export function createDefectCustomerImageModel(db: DrizzleDb): DefectCustomerImageModel {
  return new DefectCustomerImageModel(db);
}

export default DefectCustomerImageModel;
