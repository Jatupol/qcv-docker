...
import {
  ...
  IQAOQADPPMOverallRecord,
  LARDashboardRecord,
  DPPMDashboardRecord,
  UnderkillDashboardRecord,
  TopDefectRecord,
  OQADPPMOverallResult,
  HistoryTrackingRecord,
  DefectImageSummaryParams,
  DefectImageSummaryRecord,
  OQAVIRecord,
  DefectTypeAnalysisRecord,
  OverviewOQARawRecord
} from './types';
...
export class ReportModel {
   ...
   */
  async getModelsLAR(params: ReportQueryParams = { isCustomerReport: false }): Promise<string[]> {
    try {
      const {
        isCustomerReport,
        yearFrom,
      ...
      //console.log('🔧 LARReportModel.getModelsLAR called with params:', params);
      ...
      // Use LPAD to ensure ww is always 2 digits for proper string comparison
      ...
      // Select the appropriate view based on report type
      const viewName = isCustomerReport ? 'v_inspectiondata_customer' : 'inspectiondata';

      // Build WHERE clause (additional conditions after base WHERE)
      const additionalConditions = conditions.length > 0 ? `AND ${conditions.join(' AND ')}` : '';

      // Main query - join with customers_site table
      const query = `
        SELECT
      ...
      `;
      //console.log('🔧 getModelsLAR query:', query);
      //console.log('🔧 getModelsLAR values:', values);
      const result = await this.db.query(query, values);
     // console.log('🔧 LARReportModel.getModelsLAR query:', query);


      const products = result.rows.map(row => row.model);

     // console.log(`✅ getModelsLAR: Retrieved ${products.length} models`);
      return products;
  ...
  /**
   * Get models for dashboard pages (no NULL filter - returns all products)
   */
  async getModelsDashboard(params: ReportQueryParams = {}): Promise<string[]> {
    try {
      const {
        productionSites,
        customerSites,
    ...
    /**
   * Get LAR dashboard data with the provided SQL query
   * @param params - Query parameters including isCustomerReport flag
   */
  async getLARDashboard(params: ReportQueryParams = {}): Promise<LARDashboardRecord[]> {
    try {
      const {
        model,
        models,
      ...
      // Build WHERE clause for date range filtering (month_year filter)
      if (dateFrom) {
      ...
      // Build EXISTS conditions for v_inspectiondata_month filtering
      const viewConditions: string[] = ['v.model_version = TB.model'];
      ...
      const finalQuery_LAR = query.replace(/\$(\d+)/g, (_, i) => { const v = values[parseInt(i) - 1]; return Array.isArray(v) ? `ARRAY[${v.map((x: string) => `'${x}'`).join(',')}]` : `'${v}'`; });
   ...
   * @param params - Query parameters including date range and model filters
   */
  async getDPPMDashboard(params: ReportQueryParams = {}): Promise<DPPMDashboardRecord[]> {
    try {
      const {
        model,
        models,
      ...
      const finalQuery_DPPM = query.replace(/\$(\d+)/g, (_, i) => { const v = values[parseInt(i) - 1]; return Array.isArray(v) ? `ARRAY[${v.map((x: string) => `'${x}'`).join(',')}]` : `'${v}'`; });
   ...
   * @param params - Query parameters including date range and model filters
   */
  async getUnderkillDashboard(params: ReportQueryParams = {}): Promise<UnderkillDashboardRecord[]> {
    try {
      const {
        model,
        models,
      ...
      // Main Underkill dashboard query
      const query = `
        SELECT 
          U.*,
          CASE 
      ...
      const finalQuery_UK = query.replace(/\$(\d+)/g, (_, i) => { const v = values[parseInt(i) - 1]; return Array.isArray(v) ? `ARRAY[${v.map((x: string) => `'${x}'`).join(',')}]` : `'${v}'`; });
  ...
  /**
   * Get Top Defect data
   * @param params - Query parameters including date range  filters
   */
  async getTopDefect(params: ReportQueryParams = {}): Promise<TopDefectRecord[]> {
    try {
      const {
        dateFrom,
        dateTo,
      ...
      // Build WHERE clause for date range filtering (inner query)
      if (dateFrom) {
        mainConditions.push(`L.month_year >= $${paramIndex}`);
        values.push(dateFrom);
        paramIndex++;
      ...
      // Add cascading filter conditions (inner query)
      if (productionSites && productionSites.length > 0) {
        mainConditions.push(`s.site = ANY($${paramIndex})`);
        values.push(productionSites);
        paramIndex++;
      ...
      // Build model filter (outer query on t.model)
      const modelsToFilter = models && models.length > 0 ? models : (model ? [model] : []);
      if (modelsToFilter.length > 0) {
        if (modelsToFilter.length === 1) {
      ...
      // Build EXISTS clause for v_inspectiondata_month filtering
      const viewConditions: string[] = ['v.model_version = t.model'];
      ...
      const finalQuery_TD = query.replace(/\$(\d+)/g, (_, i) => { const v = values[parseInt(i) - 1]; return Array.isArray(v) ? `ARRAY[${v.map((x: string) => `'${x}'`).join(',')}]` : `'${v}'`; });
   ...
   */
  async getProductionLineHeatmap(params: ReportQueryParams = {}): Promise<any[]> {
    try {
   ...
   * @param params - Query parameters including date range
   */
  async getProductQualityScorecard(params: ReportQueryParams = {}): Promise<any[]> {
    try {
      const { dateFrom, dateTo } = params;
   ...
   * @param params - Query parameters including date range
   */
  async getDefectRootCause(params: ReportQueryParams = {}): Promise<any[]> {
    try {
      const { dateFrom, dateTo } = params;
   ...
   * @param params - Query parameters including date range
   */
  async getMonthlyQualityTrend(params: ReportQueryParams = {}): Promise<any[]> {
    try {
      const { dateFrom, dateTo } = params;
   ...
   * @param params - Query parameters including isCustomerReport flag
   */
  async getLARChart(params: ReportQueryParams = { isCustomerReport: false }): Promise<LARReportRecord[]> {
    try {
      const {
        isCustomerReport,
        model,
   ...
   * @param params - Query parameters including isCustomerReport flag
   */
  async getLARDefect(params: ReportQueryParams = { isCustomerReport: false }): Promise<LARReportRecord[]> {
    try {
      const {
        isCustomerReport,
        model,
      ...
      // Add model filter (support both single and multiple models)
      const modelsToFilter = models && models.length > 0 ? models : (model ? [model] : []);
      if (modelsToFilter.length > 0) {
        if (modelsToFilter.length === 1) {
      ...
      // Main LAR defect query with model information
      const query = `
        SELECT
          d.fy,
          d.ww,
   ...
   *                  total_ng, lar, dppm
   */
  async getIQAOQADppmOverallChart(params: ReportQueryParams = {}): Promise<IQAOQADPPMOverallRecord[]> {
    try {
      const {
        yearTo: yearParam,
        wwTo: wwParam
      ...
      // Set default values: use current fiscal year/week if null/undefined
      const year = yearParam || getCurrentFiscalYear().toString();
      const ww = wwParam || getCurrentFiscalWeek().toString().padStart(2, '0');

      console.log('🔧 IQA OQA DPPM Overall Chart called with params:', params);
      console.log('🔧 Using year:', year, 'ww:', ww);

      const values: any[] = [];
      // Always push fyww since we now have defaults
   ...
   *                  total_ng, lar, dppm
   */
  async getOQADppmOverallChart(params: ReportQueryParams = {}): Promise<OQADPPMOverallRecord[]> {
    try {
      const {
        isCustomerReport,
        yearTo: yearParam,
   ...
   * Query returns: ww, defectname, total_ng
   */
  async getOQADppmOverallDefect(params:  ReportQueryParams = {}): Promise<OQADPPMOverallRecord[]> {
    try {
      const {
        isCustomerReport,
        yearTo: yearParam,
   ...
   */
  async getSeagateIQAResult(params: ReportQueryParams): Promise<SeagateIQAResultReportRecord[]> {
    try {
      const {
        yearFrom: yearParam,
        wwFrom: wwParam
   ...
   * Query returns: ww, total_lot, total_pass_lot, total_fail_lot, total_inspection,
   *                  total_ng, lar, dppm
   * @param params - Query parameters including isCustomerReport flag and multiple models support
   */
  async getSGTIQATrendChart(params: ReportQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      ...
      // $1 = year, $2 = ww — used by both the yearmonth range subquery and the weekly filter
      const values: any[] = [year, ww];
      let paramIndex = 3;
      ...
      // Appended after an existing WHERE clause, so always starts with AND (or empty)
   ...
   * @param params - Query parameters including isCustomerReport flag and multiple models support
   */
  async getSGTIQATrendDefect(params: ReportQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      const { model, models, productFamilies, product_type, yearTo: yearParam, wwTo: wwParam, onlyWeekOfProduct } = params;
   ...
   * @param lotNumbers - Array of lot numbers to search (max 10)
   */
  async getHistoryTracking(lotNumbers: string[]): Promise<HistoryTrackingRecord[]> {
    try {
   ...
   * Returns defect names with their associated image IDs, filtered by date range and optional filters
   */
  async getDefectImageSummary(params: DefectImageSummaryParams): Promise<DefectImageSummaryRecord[]> {
    try {
      ...
      // Optional filters (array-based for multi-select)
  ...
  /**
   * Get parts filter options (product_families, product_type, model) by customer_site(s)
   * Supports cascading: customerSites → productFamilies → productTypes → models
   */
  async getPartsFilterOptions(params: {
    ...
    try {
      ...
      const conditions: string[] = ['p.customers_site = ANY($1)'];
      const values: any[] = [params.customerSites];
      let paramIndex = 2;
   ...
   * Get available models from inspectiondata
   * Returns combined model and version strings
   * @param fyww - Combined fiscal year + work week string (e.g. "202652").
   *               Returns last 52 weeks up to this fyww.
   *               If omitted, uses current fyww.
   * @param onlyWeekOfProduct - When true, returns models only for the exact fyww week.
   */
  async getSGAIQAModels(fyww?: string, onlyWeekOfProduct: boolean = false): Promise<string[]> {
    try {
   ...
   * Returns distinct product_families only (without versions), matching what OQA inspection page shows
   * @param fyww - Combined fiscal year + work week (e.g. "202652")
   */
  async getSGAIQAProductFamilies(fyww?: string): Promise<string[]> {
    try {
   ...
   */
  async getFiscalYears(report: string): Promise<string[]> {
    try {
   ...
   */
  async getWorkWeeks(report: string,fiscalYear?: string): Promise<string[]> {
    try {
  ...
  /**
   * Get Overview OQA data (lot-level rows with defect group)
   * Uses v_inspectiondata_customer + parts + customers_site (SGT) + defectdata_customer
   * Filters: station=OQA, round=1, fy, ww, optional models
   */
  async getOverviewOQA(params: ReportQueryParams): Promise<OverviewOQARawRecord[]> {
    try {
  ...
  /**
   * Get Overview OQA lot detail for a specific (fy, ww, model, defect_group)
   */
  async getOverviewOQALotDetail(fy: string, ww: string, model: string, defect_group: string): Promise<{ lotno: string; ng_qty: number; inspection_no: string }[]> {
    try {
      const query = `
   ...
   * @param models - Optional array of model names to filter (format: "model version")
   */
  async getDefectTypeAnalysis(dateFrom: string, dateTo: string, models?: string[], shifts?: string[]): Promise<DefectTypeAnalysisRecord[]> {
    try {
      const values: any[] = [dateFrom + ' 00:00:00', dateTo + ' 23:59:59.999'];
      const conditions: string[] = ['d.defect_date BETWEEN $1 AND $2'];
      let paramIndex = 3;
  ...
  async getSOQMDaily(params: ReportQueryParams): Promise<any[]> {
    try {
      const { yearFrom, wwFrom, yearTo, wwTo, models, model, customerSites } = params;

      const values: any[] = [];
      const conditions: string[] = ["L.station = 'OQA'", "L.round = 1"];
      let paramIndex = 1;

      // Fiscal year/ww range filter
      if (yearFrom && wwFrom && yearTo && wwTo) {
      ...
      // Join with parts table when customer filter is used
  ...
  async getSOQMWeekly(params: ReportQueryParams): Promise<any> {
    try {
      const { yearFrom, wwFrom, yearTo, wwTo, models, model, customerSites } = params;
      ...
      // Summary query: per-model aggregation
      const summaryQuery = `
        SELECT
          L.model || ' ' || L.version AS model,
      ...
      // Defect breakdown query: per-model per-defect
      const defectQuery = `
        SELECT
          L.model || ' ' || L.version AS model,
          f.name AS defect_name,
      ...
      // All defect types from master table
      ...
      return {
        ...
        allDefectNames: allDefectsResult.rows.map((r: any) => r.defect_name),
      };
    } catch (error) {
  ...
  async getFVIInspection(params: {
    ...
    try {
      ...
      const values: any[] = [];
      const conditions: string[] = ["L.lotno <> ''"];
      let paramIndex = 1;
   ...
   */
  async deleteFVILotInput(lotno: string): Promise<{ deletedCount: number }> {
    try {
      const result = await this.db.query(
        `DELETE FROM inf_lotinput WHERE lotno = $1`,
        [lotno]
      );
   ...
   */
  async getOQAVisualInspection(params: ReportQueryParams = { isCustomerReport: false }): Promise<OQAVIRecord[]> {
    try {
      const {
        isCustomerReport,
        model,
