...
export class ReportModel {
  ...
  async getSGTIQATrendChart(params: ReportQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      ...
      // Build WHERE clause for productFamilies filter (product_families only, without versions)
      if (productFamilies && productFamilies.length > 0) {
      ...
      // Build WHERE clause for model filter (product_families + versions)
      else {
        const modelsToFilter = models && models.length > 0 ? models : (model ? [model] : []);
        if (modelsToFilter.length > 0) {
          if (modelsToFilter.length === 1) {
  ...
  async getSGTIQATrendDefect(params:  ReportQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      ...
      // Push year and ww as separate params (matching chart query pattern)
      values.push(`${year}`);
      paramIndex++;
   ...
   */
  async getSGAIQAModels(fyww?: string): Promise<string[]> {
    try {
      console.log('🔧 LARReportModel.SGAIQAModels called with fyww:', fyww);

      // Default to current fyww if not provided
  ...
  /**
   * Get Overview OQA lot detail for a specific (fy, ww, partno_customer, defect_group)
   */
  async getOverviewOQALotDetail(fy: string, ww: string, partno_customer: string, defect_group: string): Promise<{ lotno: string; ng_qty: number; inspection_no: string }[]> {
    try {
      const query = `
