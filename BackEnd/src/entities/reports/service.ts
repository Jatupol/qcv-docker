...
export class ReportService {
   ...
   */
  async getModelsSGAIQA(fyww?: string, onlyWeekOfProduct: boolean = false): Promise<{
    success: boolean;
    data?: string[];
    message?: string;
    errors?: string[];
    ...
    try {
      console.log('🔧 LARReportService.getSGAIQA called with fyww:', fyww, 'onlyWeekOfProduct:', onlyWeekOfProduct);

      const models = await this.model.getSGAIQAModels(fyww, onlyWeekOfProduct);
   ...
   */
  async getOverviewOQALotDetail(fy: string, ww: string, model: string, defect_group: string): Promise<OverviewOQALotDetailResult> {
    try {
      if (!fy || !ww || !model || !defect_group) {
        return {
          success: false,
          message: 'fy, ww, model and defect_group are required',
          errors: ['Missing required parameters']
        };
      }

      const data = await this.model.getOverviewOQALotDetail(fy, ww, model, defect_group);
