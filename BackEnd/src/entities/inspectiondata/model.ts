...
export class InspectionDataModel {
  ...
  async getAll(searchTerm?: string, options?: {
    ...
    try {
      ...
      const result = await (this.db.$client as any).query(query, values);

      return result.rows.map((row: any) => mapDrizzleToEntity({
        id: row.id,
        station: row.station,
   ...
   */
  async getCount(searchTerm?: string, station?: string, inspectionDateFrom?: string, inspectionDateTo?: string, isCustomer?: boolean): Promise<number> {
    try {
  ...
  /**
   * Get judgment from inspectiondata_customer table by inspection_no
   */
  async getCustomerJudgment(inspectionNo: string): Promise<boolean | null> {
    try {
      const result = await this.db
        .select({ judgment: inspectiondataCustomer.judgment })
        .from(inspectiondataCustomer)
        .where(eq(inspectiondataCustomer.inspectionNo, inspectionNo))
        .limit(1);
   ...
   */
  async updateCustomerJudgment(inspectionNo: string, judgment: boolean, updatedBy: number): Promise<boolean> {
    try {
      const result = await this.db
        .update(inspectiondataCustomer)
        .set({ judgment, updatedBy, updatedAt: new Date() })
        .where(eq(inspectiondataCustomer.inspectionNo, inspectionNo))
        .returning({ inspectionNo: inspectiondataCustomer.inspectionNo });
