    ...
    constructor(service) {
        ...
        this.getLARDashboard = async (req, res, next) => {
            try {
                ...
                if (timePeriod === 'daily' && req.query.dateFrom && req.query.dateTo) {
                    const from = new Date(req.query.dateFrom);
                    const to = new Date(req.query.dateTo);
                    if (!isNaN(from.getTime()) && !isNaN(to.getTime()) && (to.getTime() - from.getTime()) / 86400000 > 30) {
                        res.status(400).json({ success: false, message: 'Daily view is limited to a 30-day date range.' });
                        return;
                    }
                }
                else if (timePeriod === 'fiscal_weekly' && req.query.dateFrom && req.query.dateTo) {
                    const fromStr = req.query.dateFrom;
                    const toStr = req.query.dateTo;
                    if (fromStr.length === 6 && toStr.length === 6) {
                        const diffWeeks = (parseInt(toStr.slice(0, 4), 10) - parseInt(fromStr.slice(0, 4), 10)) * 52
                            + (parseInt(toStr.slice(4, 6), 10) - parseInt(fromStr.slice(4, 6), 10));
                        if (diffWeeks > 13) {
                            res.status(400).json({ success: false, message: 'Weekly view is limited to a 13-week date range.' });
                            return;
                        }
                    }
                }
        ...
        this.getModelsSGAIQA = async (req, res, next) => {
            try {
                const { year, ww, fyww, onlyWeekOfProduct } = req.query;
                const fywwValue = fyww
                    || (year && ww ? `${year}${String(ww).padStart(2, '0')}` : undefined);
                const onlyWeekOfProductValue = onlyWeekOfProduct === 'true';
                console.log(`📋 GET /api/report/getModelsSGAIQA - fyww: ${fywwValue}, onlyWeekOfProduct: ${onlyWeekOfProductValue}`);
                const result = await this.service.getModelsSGAIQA(fywwValue, onlyWeekOfProductValue);
                if (result.success) {
        ...
        this.getSGTIQATrendChart = async (req, res, next) => {
            try {
                ...
                const queryParams = {
                    ...
                    onlyWeekOfProduct: req.query.onlyWeekOfProduct === 'true'
                };
                Object.keys(queryParams).forEach(key => {
                    if (queryParams[key] === undefined) {
                        delete queryParams[key];
        ...
        this.getOverviewOQALotDetail = async (req, res, next) => {
            try {
                ...
                if (!fy || !ww || !model || !defect_group) {
                    res.status(400).json({
                        success: false,
                        message: 'fy, ww, model and defect_group parameters are required'
                    });
                    return;
                }
                const result = await this.service.getOverviewOQALotDetail(fy, ww, model, defect_group);
                if (result.success) {
                    res.status(200).json(result);
                }
                else {
