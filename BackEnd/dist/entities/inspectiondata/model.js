...
class InspectionDataModel {
    ...
    async getAll(searchTerm, options) {
        try {
            ...
            if (options?.station && options.station.trim()) {
                conditions.push((0, drizzle_orm_1.eq)(schema_1.inspectiondata.station, options.station.trim()));
            }
            if (options?.inspectionDateFrom) {
                const fromDateStr = `${options.inspectionDateFrom} 00:00:00`;
                conditions.push((0, drizzle_orm_1.sql) `${schema_1.inspectiondata.inspectionDate} >= ${fromDateStr}::timestamp`);
            }
            if (options?.inspectionDateTo) {
                const toDateStr = `${options.inspectionDateTo} 23:59:59.999`;
                conditions.push((0, drizzle_orm_1.sql) `${schema_1.inspectiondata.inspectionDate} <= ${toDateStr}::timestamp`);
            }
            if (searchTerm && searchTerm.trim()) {
                const searchPattern = `%${searchTerm.trim()}%`;
                conditions.push((0, drizzle_orm_1.or)((0, drizzle_orm_1.ilike)(schema_1.inspectiondata.inspectionNo, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.station, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.shift, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.lotno, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.partsite, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.itemno, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.model, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.version, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.fvilineno, searchPattern), (0, drizzle_orm_1.ilike)(schema_1.inspectiondata.mclineno, searchPattern)));
            }
            ...
            const defectCounts = this.db
                ...
                defectNum: (0, drizzle_orm_1.sql) `count(*)::int`.as('defect_num'),
                ngNum: (0, drizzle_orm_1.sql) `COALESCE(SUM(${schema_1.defectdata.ngQty}), 0)::int`.as('ng_num')
            })
                .from(schema_1.defectdata)
            ...
            const result = await this.db
                ...
                samplingReasonName: schema_1.samplingReasons.name,
                samplingReasonDescription: schema_1.samplingReasons.description,
                defectNum: (0, drizzle_orm_1.sql) `COALESCE(${defectCounts.defectNum}, 0)`,
                ngNum: (0, drizzle_orm_1.sql) `COALESCE(${defectCounts.ngNum}, 0)`
            })
                .from(schema_1.inspectiondata)
                .leftJoin(schema_1.samplingReasons, (0, drizzle_orm_1.eq)(schema_1.inspectiondata.samplingReasonId, schema_1.samplingReasons.id))
                .leftJoin(defectCounts, (0, drizzle_orm_1.eq)(schema_1.inspectiondata.inspectionNo, defectCounts.inspectionNo))
                .where(whereClause)
    ...
    async getCount(searchTerm, station, inspectionDateFrom, inspectionDateTo) {
        try {
            const conditions = [];
            if (station && station.trim()) {
                conditions.push((0, drizzle_orm_1.eq)(schema_1.inspectiondata.station, station.trim()));
            }
