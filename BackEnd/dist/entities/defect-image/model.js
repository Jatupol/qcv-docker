...
class DefectImageModel {
    ...
    async getById(id) {
        const result = await this.db
            .select()
            .from(schema_1.defectImage)
            .where((0, drizzle_orm_1.eq)(schema_1.defectImage.id, id))
            .limit(1);
    ...
    async getAll() {
        const result = await this.db
            .select()
            .from(schema_1.defectImage)
            .orderBy((0, drizzle_orm_1.desc)(schema_1.defectImage.id));
    ...
    async getByDefectId(defectId) {
        const result = await this.db
            .select()
            .from(schema_1.defectImage)
            .where((0, drizzle_orm_1.eq)(schema_1.defectImage.defectId, defectId))
            .orderBy((0, drizzle_orm_1.asc)(schema_1.defectImage.id));
        return result.map(row => mapDrizzleToEntity(row));
    }
    async update(id, imageData) {
        const result = await this.db
            .update(schema_1.defectImage)
...
exports.default = DefectImageModel;
